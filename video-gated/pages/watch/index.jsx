...
{
  TokenBalances(input: {filter: { owner: {_eq: "0xAf1aa06FaBD863CFfe9D1DFcD2353C295a6484F5"}}, limit: 5, blockchain: polygon}) {
    TokenBalance {
      amount
      chainId
      id
...
export default function WatchStream() {
    ...
    const askContractToDepositMoney = async(amount) => {
        ...
        try {
            ...
            if (ethereum) {
                ...
                let nftTxn = await connectedContract.directDeposit(ethers.utils.parseEther("0.1"));

                console.log("Mining...please wait.")
                await nftTxn.wait();

                console.log(`Mined, see transaction: https://polygonscan.io/tx/${nftTxn.hash}`);
  ...
  // Render Methods
  const renderNotConnectedContainer = () => ( <button onClick = { connectWallet }
    ...
    <div className={styles.container}>
      ...
      <div className={styles.pageContainer}>
      {
            currentAccount === "" ? (
                ...
                button onClick = { askContractToDepositMoney }
                className = "cta-button connect-wallet-button" >
                Tip Creator </button>
            )
        }
      ...
      {error && <p>Something went wrong...</p>}
        {data && data.TokenBalances.TokenBalance.map((tok) => (
